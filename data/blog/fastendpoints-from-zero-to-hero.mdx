...
I've been building REST APIs with ASP.NET Core for years using Minimal APIs, and honestly, they've been a breath of fresh air compared to the controller-based approach. No more bloated controllers, no attribute soup, just clean, simple route handlers. It works well! 👍
...
And wow, I'm glad I did! FastEndpoints takes everything I loved about Minimal APIs and supercharges it for vertical slices. Each endpoint becomes a self-contained class with its own request, response, validation, and business logic - perfectly aligned with VSA principles. No more hunting through multiple files to understand a single endpoint. No more scattered route handlers. Just clean, focused, testable slices. 🙌
...
A quick note on **MediatR**: since moving to a commercial license, if you're using it to glue your Vertical Slice Architecture (requests, notifications, handler dispatch), FastEndpoints can usually replace it outright—built‑in request/response flow, validation pipeline, command-style endpoints, and event publishing. Dropping MediatR cuts cost, reduces moving parts, and keeps your slices lean. 💸
...
That's it! 🎉 Notice how clean this is compared to the traditional MVC setup - no controllers, no ConfigureServices method, just the essentials.
...
Now let's add proper validation using FluentValidation. FastEndpoints has this built-in! ✅
...
You might be thinking: "Isn't this just middleware?" Good question! 🤔
...
FastEndpoints has built-in support for the **Command pattern** via `ICommand` and `ICommandHandler`. This is perfect for encapsulating business logic that doesn't necessarily map to HTTP requests—like sending emails, updating caches, or triggering workflows.
...
Just call `.ExecuteAsync()` on your command instance. FastEndpoints handles the rest—finding the handler, executing it, and managing the lifecycle.

### Adding Command Middleware

Want to add cross-cutting concerns like logging, caching, or transaction management? Command middleware has you covered! 🛡️
...
✅ **Discoverability** - `ICommand` interface makes intent crystal clear  
✅ **Consistent** - Same pattern across your entire codebase

Think of commands as "mini-endpoints" for your business logic. They're perfect for operations like sending emails, generating reports, or any reusable logic that multiple endpoints might need. 🚀

## Events with Handlers

Events are the flip side of commands: **commands** say "do this thing," while **events** say "this thing happened." Events are perfect for decoupling your application—multiple handlers can react to the same event without the publisher knowing or caring. 🎯
...
Both handlers execute automatically when the event is published! No registration needed—FastEndpoints discovers them. ✨
...
And the best part? No MediatR needed! FastEndpoints provides all this out of the box. 🎉
...
Have questions or want to share your FastEndpoints experience? Drop a comment below or find me on [LinkedIn](https://www.linkedin.com/in/danieljamesmackay/)! I'd love to hear how you're using FastEndpoints in your projects. 😎
